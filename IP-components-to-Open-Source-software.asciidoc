...
* Check your source-code and dependencies for specific circumstances. Therefore check your dependencies for *NOTICE* files and verify that the entire source-code is officially owned by the copyright holder. In any such case add a *NOTICE* file to the root of repository that explains the circumstances. You can find here a https://www.apache.org/licenses/example-NOTICE.txt[sample file].
